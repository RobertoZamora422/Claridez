...
OLD_SUCCESSOR_CHECK = """OR successor_status <> 'confirmed'
                   OR successor_preparation_status <> 'preparing'"""
NEW_SUCCESSOR_CHECK = """OR NOT (
                       (successor_status = 'confirmed'
                           AND successor_preparation_status IN (
                               'preparing', 'ready', 'in_progress', 'completed'
                           ))
                       OR (successor_status = 'cancelled'
                           AND successor_preparation_status = 'cancelled')
                       OR (successor_status = 'rescheduled'
                           AND successor_preparation_status = 'rescheduled')
                   )"""

CURRENT_FUNCTION = LEGACY_FUNCTION.replace(OLD_SUCCESSOR_CHECK, NEW_SUCCESSOR_CHECK)
if CURRENT_FUNCTION == LEGACY_FUNCTION:
    raise RuntimeError("The operations guardian migration could not be assembled.")


class Migration(migrations.Migration):
    dependencies = [("scheduling", "0008_allow_terminal_reservation_successors")]

    operations = [migrations.RunSQL(CURRENT_FUNCTION, LEGACY_FUNCTION)]
